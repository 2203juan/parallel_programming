...
%       define that only the finest scale has a parent
        ...
        block1 = block(1);
        block2 = block(2);
        nblv = nv-block1+1;	% Discard the outer coefficients
        nblh = nh-block2+1;   % for the reference (centrral) coefficients (to avoid boundary effects)
        nexp = nblv*nblh;			% number of coefficients considered
        N = prod(block) + prnt; % size of the neighborhood
        
        Ly = (block1-1)/2;		% block(1) and block(2) must be odd!
        Lx = (block2-1)/2;
        ...
        L = diag(buenas)*sum(lola)/(sum(buenas)+(sum(buenas)==0));
        C_x = Q*L*Q';
